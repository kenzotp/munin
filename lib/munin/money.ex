defmodule Munin.Money do
  ...
  @doc "Keyword rules → {category, scope}. First hit wins; default other/private."
  def classify(%{} = row) do
  ...
  def list_transactions(q, scope, limit) do
    base =
  ...
  def cockpit(months \\ 6) do
    ...
    %{
      ...
      subscriptions: subscriptions(lines),
      missing_receipts: missing_receipts(lines),
  ...
  defp subscriptions(lines) do
    lines
    |> Enum.filter(&(&1.amount_cents < 0))
    |> Enum.group_by(&{normal_payee(&1), &1.amount_cents})
    |> Enum.map(fn {{payer, cents}, ls} ->
      months = ls |> Enum.map(&{&1.booked_at.year, &1.booked_at.month}) |> Enum.uniq() |> length()
      if months >= 3, do: %{payer: payer, cents: cents, months: months, scope: hd(ls).scope}, else: nil
    end)
  ...
  def eur(year) do
    lines =
      Repo.all(
        from t in Transaction,
          where: t.scope == "business" and fragment("extract(year from ?)", t.booked_at) == ^year
      )
